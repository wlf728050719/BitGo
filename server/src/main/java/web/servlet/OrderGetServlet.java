...
public class OrderGetServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp){
        try {
            ...
            switch (method){
                case "getBuyOrders":getBuyOrders(map,resp);break;
                case "getSellOrders":getSellOrders(map,resp);break;
    ...
    private void getBuyOrders(Map<String,String> map, HttpServletResponse resp) throws IOException{
        String buyerId = map.get("userId");
        Result result = orderService.getOrdersByBuyerId(Integer.parseInt(buyerId));
        resp.getWriter().write(new ObjectMapper().writeValueAsString(result));
    }
    private void getSellOrders(Map<String,String> map,HttpServletResponse resp) throws IOException {
        String sellerId = map.get("userId");
        Result result = orderService.getOrdersBySellerId(Integer.parseInt(sellerId));
        resp.getWriter().write(new ObjectMapper().writeValueAsString(result));
    }
